...
-(void)readInProgress{
    for (int i =0 ; i<[toDoArray count]; i++) {
        if ([[[toDoArray objectAtIndex:i] taskState ]isEqualToString:@"In Progress"]) {
            [inProgressArray addObject:[toDoArray objectAtIndex:i]];
...
-(void)deletetask:(NSInteger)index{
    //must be change here
    [inProgressArray removeObjectAtIndex:index];
    //[self saveData];
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText{
    if (searchText.length == 0) {
        ...
        for (int i=0 ; i<[inProgressArray count];i++) {
            NSRange range = [[[inProgressArray objectAtIndex:i] taskName] rangeOfString:searchText options:NSCaseInsensitiveSearch];
            if (range.location != NSNotFound) {
                [searchArray addObject:[inProgressArray objectAtIndex:i]];
            }
        }
